...
def _make_norm_bn(num_features):
  return nn.BatchNorm2d(num_features, eps=NORM_BN_EPSILON,
                        momentum=NORM_BN_MOMENTUM, affine=True)
...
class Predictron(nn.Module):
  ...
  def forward(self, x):
    ...
    rewards = torch.cat(
      [torch.zeros(b, 1, ms, device=device), rewards], dim=1)

    # gammas: [B, K, ms] -> prepend ones -> [B, K + 1, ms]
    gammas = torch.stack(gammas_arr, dim=1)
    gammas = torch.cat(
      [torch.ones(b, 1, ms, device=device), gammas], dim=1)
...
def train(args):
  ...
  for step in range(args.max_steps):
    ...
    if step % 10 == 0:
      ...
      logger.info(format_str % (datetime.datetime.now(), step, loss_value,loss_preturns.item(), loss_lambda_preturns.item(),examples_per_sec, sec_per_batch))
...
def parse_args():
  ...
  p.add_argument('--max_grad_norm', type=float, default=10., help='clip grad norm into this value')
