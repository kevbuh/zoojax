...
class SEBottleneck(nn.Module):
    ...
    def __call__(self, x, train: bool):
        ...
        y = conv(self.width, (1, 1), strides=(self.stride, self.stride), name="conv_reduce")(x)
        ...
        residual = x
        if self.proj:
            residual = conv(self.out_channels, (1, 1), strides=(self.stride, self.stride), name="conv_proj")(x)
...
class SEResNet(nn.Module):
    ...
    def __call__(self, x, train: bool = False):
        norm = partial(nn.BatchNorm, use_running_average=not train, momentum=0.9, epsilon=1e-5)
        # Stem.
        y = nn.Conv(64, (7, 7), strides=(2, 2), padding=((3, 3), (3, 3)), use_bias=False, kernel_init=MSRA, name="stem_conv")(x)
        ...
        for i, (nb, w) in enumerate(zip(self.stage_blocks, self.stage_widths)):
            ...
            for b in range(nb):
                ...
                y = SEBottleneck(width=w, out_channels=out, stride=stride, reduction=self.reduction, proj=(b == 0), name=f"conv{i + 2}_{b + 1}")(
                    y, train
                )
...
ARCHS = {"se_resnet50": se_resnet50, "se_resnet101": se_resnet101, "se_resnet152": se_resnet152}
...
def make_optimizer(args, steps_per_epoch):
    schedule = make_lr_schedule(args.base_lr, steps_per_epoch, args.lr_step_epochs, args.lr_gamma, args.epochs)
    ...
    return optax.chain(
        optax.add_decayed_weights(args.weight_decay, mask=_decay_mask), optax.sgd(learning_rate=schedule, momentum=args.momentum, nesterov=False)
    )
...
def make_imagenet_loader(root, batch_size, crop_size, train):
    ...
    def collate(samples):
        ...
        return imgs, labels
    return torch.utils.data.DataLoader(ds, batch_size=batch_size, shuffle=train, num_workers=4, drop_last=train, collate_fn=collate)
...
def train(args):
    print(
        f"# faithful SENet training: {args.arch}, batch={args.batch_size}, "
    ...
    if args.data_dir:
        train_loader = make_imagenet_loader(f"{args.data_dir}/train", args.batch_size, args.crop_size, train=True)
        ...
        for epoch in range(args.epochs):
            ...
            print(f"epoch {epoch:3d}  loss {float(loss):.4f}  acc {float(acc):.3f}  " f"({time.time() - t0:.0f}s)")
        return state
    ...
    n_params = sum(p.size for p in jax.tree_util.tree_leaves(state.params))
    print(f"# {n_params:,} parameters; no data_dir -> synthetic smoke run " f"({args.smoke_steps} steps, batch={images.shape[0]})")
...
def test_batchnorm_stats_update_in_train_mode():
    ...
    _, new_state = model.apply({"params": params, "batch_stats": batch_stats}, x, train=True, mutable=["batch_stats"])
...
def test_lr_schedule_step_decay():
    ...
    spe = 5
    sched = make_lr_schedule(0.1, steps_per_epoch=spe, step_epochs=30, gamma=0.1, total_epochs=100)
